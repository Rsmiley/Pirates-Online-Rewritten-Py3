...
class Explosion(PooledEffect, EffectController):
    ...
    def createTrack(self):
        self.track = Sequence(Func(self.p0.setBirthRate, 0.02), Func(self.p0.clearToInitial), Func(self.f.start, self, self.particleDummy), Func(self.f.reparentTo, self), Wait(0.6), Func(self.p0.setBirthRate, 100), Wait(1.25), Func(self.cleanUpEffect))
    ...
    def setEffectScale(self, scale):
        ...
        if self.p0:
            ...
            self.p0.renderer.setFinalYScale(0.1 * self.cardScale * self.effectScale)
